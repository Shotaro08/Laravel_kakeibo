                        ...
                        <div class="lg:w-2/3 w-full mx-auto overflow-auto">
                            <div class="flex justify-end">
                                <button onclick="location.href='{{ route('user.create') }}'"
                                    class="flex sm:ml-auto md:ml-auto text-white bg-blue-500 border-0 py-1 px-3 md:py-2 md:px-6 focus:outline-none hover:bg-blue-600 rounded mb-3 sm:text-sm md:text-base">新規登録</button>
                            </div>
                            <table class="table-auto w-full text-center whitespace-no-wrap">
                                ...
                                <tbody>
                                    @foreach ($e_main as $e)
                                        <tr>
                                            <td class="px-1 md:px-4 py-3 text-xs md:text-base">{{ $e->month }}</td>
                                            <td class="px-1 md:px-4 py-3 text-xs md:text-base">{{ $e->date }} </td>
                                            <td class="px-1 md:px-4 py-3 text-xs md:text-base text-gray-900">
                                                {{ number_format($e->amount) }}</td>
                                            <td class="px-1 md:px-4 py-3 text-xs md:text-base">{{ $e->description }}</td>
